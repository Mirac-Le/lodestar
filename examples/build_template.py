...
列定义历经几轮迭代：
  ...
  v2（2026-04-20 上午）+2：合作价值（0-5） / 资源类型
  v3（2026-04-20 下午）−1：删掉「关系类型」列。「是否联系到」这件事
       完全可以从「可信度」单列推断（0=未联系、1-5=已联系），多一列
       反而制造了二义性（「关系类型=已联系 + 可信度=0」是矛盾态）。
       一列承载所有事实信息，模板心智负担降到最低：每个人就一个数字。
...
def build() -> Path:
    ...
    for row_i, row in enumerate(SAMPLE_ROWS, start=1):
        # Sole signal for "未联系": 可信度 == 0. Color the row金 if so,
        # else green for "已联系".
        is_uncontacted = row[STRENGTH_COL] == 0
        if is_uncontacted:
    ...
    tips: list[tuple[str, str]] = [
        ...
        ("★★ 单列承载一切：可信度 0-5",
         ...
         "  0 → 未联系：我还没直接接触过这个人。系统不建 Me 边，"
         "      搜索时若命中，会自动算 1-3 跳引荐路径并放在『需要引荐』段。\n"
         "  1 → 点头之交：刚加微信、饭局认识、被介绍认识但没深聊。\n"
         "  3 → 普通朋友：能直接拉群、互相有印象。\n"
         "  5 → 核心铁磁：随叫随到、深度合作、信任无保留。\n"
         "⚠️ 留空 = 默认 3，所以**未联系的人务必显式填 0**，否则会被当成"
         "普通朋友来建直边，引荐路径就推不出来了。"
         "\n⚠️ **不要预先在表里标『谁是我想接触的人』**。"
         "『下次想找谁』是 web 端搜索框里那句自然语言决定的，由 LLM 解析意图后"
         "从全表（含已联系 + 未联系）一起匹配——任何人都可能是某次查询的最佳匹配。"),
        ("★★ AI 自动补字段（v2 新机制）",
...
if __name__ == "__main__":
    ...
    print(f"  · {len(SAMPLE_ROWS)} sample rows  (可信度 5 / 4 / 0)")
