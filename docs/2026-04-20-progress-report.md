...
> 今日交付 10 项：① 双 owner 切换（Richard / Tommy） ② LLM L1 结构化抽取（公司 / 城市 / 职务 / 标签） ③ 公司名脱敏：把云端可见面从「全量公司」缩到「未结构化的新公司」 ④ 前端 3 个 AI 入口 ⑤ `infer-colleagues` 子命令把 LLM 抽出的公司物化成 peer 边 ⑥ 关系来源的 provenance 优先级链 ⑦ **`normalize-companies` 公司名归一化（builtin + 用户 alias 文件 + 可选 LLM 聚类）** ⑧ **关系类型术语统一：「目标」→「未联系」，importer 不再向后兼容旧词** ⑨ **关系类型最终收成二态：`已联系 / 未联系`，废除冗余的 `弱认识` 档位（与可信度 1-5 重复表达），把"事实"和"意图"两个维度彻底分开** ⑩ **更进一步：把「关系类型」列彻底删除，「是否联系到」完全从「可信度」(0=未联系，1-5=已联系) 单列推断 —— 模板从 13 列变 12 列，再无字段间冲突的可能**
...
| **「目标」→「未联系」全仓统一** | 关系类型档位之前叫"目标"，会让人误以为它就是搜索语义上的"目标人物"。其实搜索本身完全不偏向它（PathFinder 早就不再把 `is_wishlist` 当 ranking 信号），它只决定**要不要建 Me 边**——也就是"我有没有直接联系到这个人"。今天把这个档位重命名为「未联系」，UI / 模板 / 文档全部对齐。importer **明确不向后兼容**：旧值（"目标" / "target" / "想认识" / "陌生" / "未认识"）不再被识别为 wishlist，会按默认的 `已联系` 处理，并加了回归测试守住这条线。这样填表人和提示词里都不会再有"目标 = 想要找的人"的歧义。 |
| **关系类型收成二态：`已联系 / 未联系`，废除 `弱认识`** | 进一步反思后发现 `弱认识` 这一档**本质上就是 `可信度=1` 的快捷方式**——可信度列已经能用 1-5 连续表达"有多熟"，再开一个离散的"弱"档就是双重表达 + 让填表人困惑。今天把模板改成只有两个值：`已联系`（默认，按可信度 1-5 建 Me 边）和 `未联系`（不建 Me 边）。importer 中的 `_KIND_WEAK` 常量与 `if kind == WEAK: strength = 1` 的特殊路径全部删除；任何写了 `弱认识` 的旧表行会 silent fall through 到 `已联系` 默认，可信度照填、行为不变。这条改动等价于一次设计正交化：**事实**（联系/未联系）∥ **强度**（可信度 1-5）∥ **意图**（搜索框那句话）三个维度彻底分开，没有任何字段同时表达多个维度。 |
| **再下一刀：把「关系类型」列整个删除** | 收成二态之后又发现：既然只有两个值、且「已联系」和「可信度 ≥ 1」一一对应、「未联系」就是「可信度 = 0」，那这一列**就是「可信度」的二值投影**——纯冗余。今天 (v3) 把「关系类型」列从模板、importer、文档、demo 网络全部移除。新模型：**「可信度」一列承载所有事实**，0 = 未联系（不建 Me 边、走引荐）、1-5 = 已联系（数字即边强度）。importer 同步：`_parse_strength` 把允许范围从 `[1,5]` 放宽到 `[0,5]`（之前 0 会被 clamp 成 1，会偷偷把未联系变成弱认识）；`_normalize_kind` / `_KIND_*` 全删；`is_wishlist = (strength == 0)` 一行 derive。老 xlsx 里残留「关系类型」列会被 silently 忽略（行为完全由「可信度」驱动），不需要先清表。最大收益是**填表心智极简**——每个人就一个 0-5 的整数，不可能再出现"关系类型=已联系 + 可信度=0"这种自相矛盾的合法填法。 |
...
- [x] 二态 Me 边声明，**单列驱动**：`可信度=0` → 未联系（不建 Me 边、走多跳引荐）/ `可信度=1-5` → 已联系（数字即边强度）；模板从 13 列变 12 列 ⭐今日
