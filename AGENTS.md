...
- 2026-04-21 gold queries × 20 真 hybrid 评测（`docs/eval_2026-04-21.md`，前提是补完 embedding）：`bge`（本地 cross-encoder）与 `llm`（qwen-plus 重排）质量基本打平（R@5 0.766 vs 0.769、NDCG@10 0.753 vs 0.782），但 `bge` 延迟仅 11.7s/q vs `llm` 45.3s/q（**4× 快**）且**零 token 成本**，cliff-avoid 还反超（0.85 vs 0.80）——若已装 `rerank` extra，默认推荐 `LODESTAR_RERANKER=bge`，`llm` 仅在 MRR 略胜（0.900 vs 0.842）的高精度场景再考虑。
