...
- 路径搜索引入软阈值 `Settings.weak_me_floor`（默认 2，可经 `LODESTAR_WEAK_ME_FLOOR` 改）：strength < floor 的 **Me 边**在 `PathFinder._build_graph` 里乘 `max_hops² × 4` 的惩罚倍率，shortest_path 因此优先走更熟的多跳引荐；只有当不存在替代路径时才回退到该弱边并标 `path_kind=weak`。`path_kind` 现在按**实际选中的路径**分类（1 跳 + strength≥floor → direct；1 跳 + strength<floor → weak；多跳 → indirect），不再按"Me 边是否存在"硬判定。
