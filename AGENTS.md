...
- Stage-2 重排 `rerank` extra 的依赖约束（`pyproject.toml` 已固化）：`FlagEmbedding 1.3.x` 在 `__init__` 里 eager 加载 MiniCPM layerwise reranker，会 import `transformers.utils.import_utils.is_torch_fx_available`——该符号在 `transformers 5.0` 已移除，所以必须把 `transformers` 钉在 `>=4.45,<5.0`。另 `torch` 通过 `[[tool.uv.index]] name="pytorch-cpu"` + `[tool.uv.sources] torch = [{ index = "pytorch-cpu" }]` 钉到 PyTorch CPU wheel，避免在无显卡机器上拉 ~2GB 的 `nvidia-cublas` / `nvidia-cudnn-cu13` 全家桶。
- 国内拉 HuggingFace 模型权重（如 `BAAI/bge-reranker-v2-m3`）需同时设 `HF_ENDPOINT=https://hf-mirror.com` **和** `HF_HUB_DISABLE_XET=1`：`hf-mirror.com` 不代理 HF 1.x 的 Xet CDN（`cas-bridge.xethub.hf.co`），不关 Xet 的话大文件 chunk 仍会回源超时。
- `uv sync` 用国内 PyPI 镜像（如阿里云）应配置在**用户级 `~/.config/uv/uv.toml`**（用裸 `[[index]] default = true`），不要写进仓库 `pyproject.toml`：pyproject 里只认 `[[tool.uv.index]]`，裸 `[[index]]` 会被解析成顶层数组、对 uv 是 no-op，徒增噪音又强制队友走镜像。
- 2026-04-21 gold queries × 20 真 hybrid 评测（`docs/eval_2026-04-21.md`，前提是补完 embedding）：`bge`（本地 cross-encoder）聚合质量小幅领先 `llm`（qwen-plus 重排）（R@5 0.766 vs 0.734、NDCG@10 0.753 vs 0.738、cliff-avoid 0.85 vs 0.80），延迟 11.7s/q vs 17.5s/q——若已装 `rerank` extra，默认推荐 `LODESTAR_RERANKER=bge`，`llm` 仅在「找具体角色 + 强语义判断」类单 query（如「我想找量化私募老板」，`llm` top-5 全本人、`bge` 会混进噪声）需要时临时切。
- `LLMJudgeReranker` 的 prompt **不要**让 LLM 输出 `reason` 字段：旧版每个候选要写一句中文判定理由，30 候选下输出 ~1700 token、qwen-plus 30-40 tok/s 直接吃掉 40-50s 延迟；下游程序根本不读 `reason`（`_JudgeVerdict.reason` 在整个 src/ 没有任何 reader）。2026-04-21 砍掉后 latency 从 45.3s/q → 17.5s/q（-61%），单 query 质量没退甚至更纯（top-5 桥梁被压下去）。改 LLM 结构化输出 prompt 时统一原则：**只让 LLM 输出下游真正消费的字段，每多一个字段就在每个候选上付一遍输出 token 的钱**。
- **跑搜索 / 评测前必查 `vec_person_bio` 是否填满**：importer / 新建联系人若没显式跑 embedding，`vec_person_bio` 就是空表，`HybridSearch` 三路里 vector 通道会**完全静默失效**——所有 reranker 对比和 R@5 都会带"keyword-only"的系统偏差（一次踩过：4 条 R@5=0 死角的根因就是这个，补 `uv run lodestar reembed` 后全部回阳）。诊断脚本 `scripts/debug_zero_recall.py` 拆三路输出到 `docs/zero_recall_2026-04-21.md`，能一眼看出 vec 通道是否在工作。
